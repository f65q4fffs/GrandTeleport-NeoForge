...
This project is an unofficial port of the original Fabric mod **Grand Teleport (GTP)** by **hookuru_** to the **NeoForge** ecosystem (Minecraft 1.21.1).
...
Grand Teleport adds a GTA V-style cinematic camera transition when teleporting in Minecraft.
...
* `/gtp status` or `/grandtp status` - Show the current mod status.
* `/gtp player_freeze on|off|status` - Enable/disable/check the player freeze status during transitions.
